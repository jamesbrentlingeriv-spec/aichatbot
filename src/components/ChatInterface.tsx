...
export function ChatInterface({
  ...
  useEffect(() => {
    ...
    // Show full message immediately — no artificial character-by-character delay
    setStreamingRevealed(lastMsg.content.length);
    streamingTimerRef.current = null;

    // No cleanup needed — no interval running
  ...
  return (
    <div className="flex-1 flex flex-col h-full min-w-0 theme-bg relative">
      ...
      {/* Chat Header */}
      <div className="relative z-10 flex items-center justify-between px-3 md:px-6 py-3 md:py-4 border-b theme-border theme-header-bg shrink-0 min-h-14">
            ...
            <button
              ...
              className={`px-2.5 md:px-3 py-1.5 text-[11px] md:text-xs font-medium rounded-lg transition-colors flex items-center gap-1.5 ${
                isVoiceActive
                  ...
                  : 'theme-bg-secondary text-green-400 hover:text-green-300 border border-green-800 hover:border-green-700 hover:bg-green-950'
              }`}
              title={isVoiceActive ? 'Phone call active' : 'Start phone call'}
            >
              ...
              {isVoiceActive ? 'On Call' : 'Phone Call'}
            </button>
          )}
          {messages.length > 0 && (
            <button
      ...
      {conversations.length > 0 && (
        <div className="relative z-10 flex items-center gap-1 px-2 md:px-3 py-2 border-b theme-border overflow-x-auto shrink-0 bg-black/40 backdrop-blur-sm">
      ...
      {/* Chat Messages */}
      <div className="relative z-10 flex-1 overflow-y-auto chat-scroll px-3 md:px-4 py-3 md:py-4 pb-56 md:pb-4 space-y-3 md:space-y-4">
        {messages.length === 0 && (
      ...
      {/* Fixed Input Area */}
      <div className="relative z-10 sticky bottom-0 left-0 right-0 border-t theme-border theme-bg px-2 md:px-4 py-2 md:py-3 shrink-0">
        {attachedPreview && (
